...
%Options of the search algorithms    
    ...
    parfor i_sim=1:input.N_sim
           ...
           myfun=@(pp_component) (system_beta_1yr_given_p_component( pp_component,input,i_sim ) - beta_system_req).^2;
           %[p_out]=fminsearch(myfun,p_start,options_fminsearch);
           p_component(i_sim)=fminunc(myfun,p_start,options_fminunc);
           %Check
           p_startt=p_start;
           while system_beta_1yr_given_p_component( p_component(i_sim),input,i_sim )>beta_system_req+.05%check
               p_startt=p_startt-0.5;
               disp('The design of the system was not successful \beta_{sys}~=\beta_{sys_req}')
               disp(system_beta_1yr_given_p_component( p_component(i_sim),input,i_sim ))
