...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Details: 
% This scripts optimizes yearly target relaibilitity at component and 
...
% Define the domain where the total Expected costs are to be estiamted
    dom_beta_min=3.5; %minimum value of relaibility index
    dom_beta_max=5.5; %maximum values of relaibility index
...
% Find minimum of the spline interpolant    
    ...
    text(beta_opt_level1-0.5,ExpCost_opt,strcat('\beta_{c,opt}^{(1a )} = ',num2str(0.01*round(100*beta_opt_level1))),'Color','blue')
    xlabel('\beta_{c}^{(1a )}'); ylabel('E_{\Delta,\Theta}[C_t_o_t]');grid on; title('Level 1 Code')
...
% Find minimum of the spline interpolant     
    ...
    text(beta_opt_level3-0.5,ExpCost_opt,strcat('\beta_{sys,opt}^{(1a )} = ',num2str(0.01*round(100*beta_opt_level3))),'Color','blue')
    xlabel('\beta_{sys}^{(1a )}'); ylabel('E_{\Delta,\Theta}[C_t_o_t]');grid on; title('Level 3 Code')
...
% Check robustness index of structures designed with  beta_opt_level1
    ...
    parfor i_sim=1:input.N_sim
        beta_sys_lifetime_with_beta_opt_level1(i_sim) = system_beta_given_p_component( p_component_with_beta_opt_level1(i_sim),input,i_sim )
        beta_sys_yearly_with_beta_opt_level1(i_sim) = system_beta_1yr_given_p_component( p_component_with_beta_opt_level1(i_sim),input,i_sim )
    end
% Check robustness index of structures designed with  beta_opt_level3
    ...
    parfor i_sim=1:input.N_sim
        beta_comp_lifetime_with_beta_opt_level3(i_sim) = component_beta_given_p_component( p_component_with_beta_opt_level3(i_sim),input,i_sim )
        beta_comp_yearly_with_beta_opt_level3(i_sim) = component_beta_1yr_given_p_component( p_component_with_beta_opt_level3(i_sim),input,i_sim )
    end
% plots Robustness
    figure('Name',name_case)
    subplot(2,1,2)
    histogram(robustness_index_with_beta_opt_level1~=-999)
    grid on; xlabel('I_r_o_b');ylabel('f_{I_r_o_b}'); title('Design with \beta_{c,opt}^{(1a )}')
    subplot(2,1,1)
    histogram(robustness_index_with_beta_opt_level3~=-999)
    grid on; xlabel('I_r_o_b');ylabel('f_{I_r_o_b}'); title('Design with \beta_{sys,opt}^{(1a )}')
% plots beta 
    figure('Name',name_case)
    subplot(2,2,2)
    histogram(beta_sys_lifetime_with_beta_opt_level1)
    grid on; xlabel('\beta_{sys}^{(T_l_i_f_e )}');ylabel('f_{B_s_y_s}'); title('Design with \beta_{c,opt}^{(1a )}')
    subplot(2,2,1)
    histogram(beta_comp_lifetime_with_beta_opt_level3)
    grid on; xlabel('\beta_c^{(T_l_i_f_e )}');ylabel('f_{B_c}'); title('Design with \beta_{sys,opt}^{(1a )}')
    subplot(2,2,4)
    histogram(beta_sys_yearly_with_beta_opt_level1)
    grid on; xlabel('\beta_{sys}^{(1a )}');ylabel('f_{B_s_y_s}'); title('Design with \beta_{c,opt}^{(1a )}')
    subplot(2,2,3)
    histogram(beta_comp_yearly_with_beta_opt_level3)
    grid on; xlabel('\beta_c^{(1a )}');ylabel('f_{B_c}'); title('Design with \beta_{sys,opt}^{(1a )}')
